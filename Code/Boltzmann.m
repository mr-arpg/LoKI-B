...
%       boltzmann.ionTemporalGrowth = zeros(boltzmann.energyGrid.cellNumber);
  ...
  methods (Access = private)
    ...
    function evaluateContinuousOperators(boltzmann)
      ...
      % updated for variable energy grid
    ...
    function evaluateFieldOperator(boltzmann, ~, ~)
      ...
      % evaluate operator elements - variable energy grid
      
      % evaluate diagonal elements
      % initial expression - uniform energy grid
...
%       boltzmann.fieldMatrix(1:cellNumber+1:cellNumber*cellNumber) = ...
%         - (boltzmann.g_E(1:cellNumber)+boltzmann.g_E(2:cellNumber+1))./energyStep.^2;
      ...
      % alternative expression for variable energy grid
      % boltzmann.fieldMatrix(1:cellNumber+1:cellNumber*cellNumber-1) = ...
      %   - boltzmann.g_E(2:cellNumber).*weightGrid(1:cellNumber-1).*2/energyStep(1:cellNumber-1)^2;
      % boltzmann.fieldMatrix(cellNumber+2:cellNumber+1:cellNumber*cellNumber) =  ...
      %     boltzmann.fieldMatrix(cellNumber+2:cellNumber+1:cellNumber*cellNumber) - ...
      ...
      % for debugging purposes
%       boltzmann.fieldMatrix(2:cellNumber+1:cellNumber*cellNumber) = ...
%         boltzmann.g_E(2:cellNumber)./energyStep(2:cellNumber).^2;
      ...
      % evaluate superior diagonal elements
      % for debugging purposes - uniform energy grid
%       boltzmann.fieldMatrix(cellNumber+1:cellNumber+1:cellNumber*cellNumber) = ...
%         boltzmann.g_E(2:cellNumber)./energyStep(2:cellNumber).^2;
    ...
    function evaluateElasticOperator(boltzmann)
      ...
      % evaluate factors - uniform energy grid
...
%       factor2 = (kb*Tg/boltzmann.energyGrid.step-0.5)/boltzmann.energyGrid.step;
      ...
      % for debugging purposes
%       factor1 = (kb*Tg/energyStep(1)+0.5)/energyStep(1);
%       factor2 = (kb*Tg/energyStep(1)-0.5)/energyStep(1);
...
%         - (boltzmann.g_c(1:cellNumber)*factor1+boltzmann.g_c(2:cellNumber+1)*factor2);
     ...
     % evaluate inferior diagonal elements
      % debugging purposes - uniform energy grid
%       boltzmann.elasticMatrix(2:cellNumber+1:cellNumber*cellNumber) = boltzmann.g_c(2:cellNumber).*factor2;

      % evaluate inferior diagonal elements
      % initial expression - variable energy grid
...
%         boltzmann.attachmentConservativeMatrix);
    ...
    function eedf = solveSpatialGrowthMatrix(boltzmann)
      ...
      %  ... assuming that there is no electron density gradient: alpha/N = CI/vd
      %  ... assuming that the discriminant is zero: alpha/N = 2*CI/vd 
      if muE^2-4*CI*ND < 0
...
%       end
    ...
    function swarmParam = evaluateSwarmParameters(boltzmann)
      ...
      % evaluate reduced energy mobility - variable energy grid
      swarmParam.redMobilityEnergy = -gamma/3*sum(energyNode(2:N).^2.*(eedfLocal(2:N)-eedfLocal(1:N-1))./...
        totalCrossSectionAux(2:N));
...
% direct inverion (mldivide matlab routine) and returns the corresponding eedf
  ...
  energyStep = energyGrid.energyStep;
